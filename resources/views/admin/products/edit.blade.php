  ...
  <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      @if($product->thumbnail)
        <div class="mb-2">
          <img src="{{ Storage::url($product->thumbnail) }}" class="img-fluid rounded border" style="max-height:160px" alt="Thumbnail">
        </div>
      @endif
    ...
    <div class="mb-3">
      ...
      <textarea name="long_description" id="long_description" rows="8" class="form-control">{{ old('long_description',$product->long_description) }}</textarea>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const genBtn = document.getElementById('generateSlug');

  // Fungsi pembentuk slug
  ...
  // Deteksi apakah slug diubah manual
  let manuallyEdited = false;

  slugEl.addEventListener('input', () => {
    manuallyEdited = slugEl.value.trim().length > 0;
  });

  nameEl.addEventListener('input', () => {
    if (!manuallyEdited) slugEl.value = slugify(nameEl.value);
  });
